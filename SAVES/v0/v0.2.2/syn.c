...

{
   ...
   int iStderrFlag = FALSE;
   /* Flag if stderr redirection. */
   ...
   for (i = 1; i < iLength; i++)
   {
      ...
      /* stdin redirection. */
	       ...
	       (strcmp(Lex_returnValue(psToken), "0<") == 0))
      {
      ...
      /* stdout redirection. */
	       ...
	       (strcmp(Lex_returnValue(psToken), "1>") == 0))
      {
         /* Multiple redirection. */
         if (iStdoutFlag)
         {
      ...
      /* stderr redirection. */
      else if (strcmp(Lex_returnValue(psToken), "2>") == 0)
      {
         /* Multiple redirection. */
         if (iStderrFlag)
         {
   ...
   if (!iStderrFlag)
   {
      psCmd = Syn_makeCmd(CMD_STDERR, NULL);
      if (psCmd == NULL)
      {
         fprintf(stderr, "%s: cannot allocate memory\n", pcProgName);
         return FALSE;
   ...
   else
   {
      ...
      psToken = (Token_T)DynArray_get(oTokens, iStderrFlag);
      /* Quoted file name added without check. */
      if (Lex_returnType(psToken) == TOKEN_QUOTE)
      {
         psCmd = Syn_makeCmd(CMD_STDERR, Lex_returnValue(psToken));
         if (psCmd == NULL)
         {
            fprintf(stderr, "%s: cannot allocate memory\n", pcProgName);
            return FALSE;
